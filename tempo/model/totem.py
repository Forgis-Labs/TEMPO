...
class TOTEMTokenizer(nn.Module):
    ...
    def from_pretrained(cls, checkpoint_path: str, device: str = "cpu") -> TOTEMTokenizer:
        ...
        model = cls(
            num_hiddens=config.get("block_hidden_size", 64),
            num_residual_layers=config.get("num_residual_layers", 2),
            num_residual_hiddens=config.get("res_hidden_size", 128),
        ...
        # Map TOTEM checkpoint keys to our module keys
        key_map = {}
        for k in state_dict:
            new_k = k
            if k.startswith("vq."):
        ...
        mapped = {key_map.get(k, k): v for k, v in state_dict.items()}

        # Load encoder, quantizer, and decoder weights
